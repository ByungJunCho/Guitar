...
기타 튜너, 연습 플레이어, 메트로놈, AI 코치, **타브악보 변환기**, **오선지 악보 변환기**, **코드 연습**, **코드 시험**을 하나로 묶은 웹 앱.
...
| **코드 연습** | 마이크로 실시간 코드 감지 + 다이어그램 안내, 일치도 피드백 |
| **코드 시험** | 힌트 없이 5문제 랜덤 코드 테스트, 점수 결과 확인 |
...
1. 메인 메뉴에서 **코드 연습** 버튼 클릭
2. 상단 그리드에서 연습할 코드를 선택 (클릭 시 손가락 다이어그램 + 운지법 미리보기)
3. **연습 시작** 버튼 클릭 → 마이크 권한 허용
4. 화면에 표시된 코드를 기타로 연주하면:
   - **타겟 코드 일치도** 바가 실시간으로 반응
   - 0.5초 이상 정확히 유지하면 자동으로 다음 코드로 넘어감
   - **다음 코드 건너뛰기** 버튼으로 강제 이동 가능

> 다이어그램에서 `×` = 안침, `○` = 개방현, `●` = 프렛 위치를 나타냅니다.

---

## 코드 시험 사용법

1. 메인 메뉴에서 **코드 시험** 버튼 클릭
2. **시험 시작** 버튼 클릭 → 마이크 권한 허용
3. 다이어그램·힌트 없이 코드 이름만 표시됨 → 직접 연주하여 통과
4. 총 5문제 랜덤 출제, 상단에 **점수(0/5)** 실시간 표시
5. 5문제 모두 통과하면 결과 오버레이(만점 달성!) 표시 후 다시 시험 보기 가능
